...
# def test(epoch, test_loader, device, model, optimizer, DTANargs):
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            if len(t_test_op) <= 0:
                t_test_op = output.data.cpu().numpy()
                # t_test_theta = theta
            else:
                t_test_op = np.concatenate((t_test_op, output.data.cpu().numpy()), axis=0)
                # t_test_theta = torch.cat((t_test_theta, theta), dim=0)
