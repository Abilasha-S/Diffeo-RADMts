...
class GDN(nn.Module):
    ...
    def forward(self, data, org_edge_index):
        ...
        for i, edge_index in enumerate(edge_index_sets):
            ...
            # weight_arr=torch.randn(node_num,node_num).repeat(1,batch_num).to(device)

            gcn_out = self.gnn_layers[i](x, batch_edge_index, weights=self.weight_arr.repeat(batch_num, 1), node_num=node_num*batch_num)
            # gcn_out = self.gnn_layers[i](x, batch_edge_index, weights=self.weight_arr.repeat(batch_num, 1).view(-1, self.dim).contiguous(), node_num=node_num*batch_num)
